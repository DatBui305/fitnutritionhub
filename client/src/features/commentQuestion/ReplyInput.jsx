...
const ReplyInput = ({ qid, accessToken, addReply, cid, changeInput }) => {
  ...
  const handleComment = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/v1/question/comment/${qid}/reply/${cid}`,
        { comment },
        {
          headers: {
      ...
      if (response.data && response.data.question && response.data.question.comments) {
        // Find the comment by cid to get the updated replies array
        const updatedComment = response.data.question.comments.find(

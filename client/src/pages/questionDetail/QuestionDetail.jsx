import React, { useState, useEffect, useContext } from "react";
import axios from "axios";
import { useParams } from "react-router-dom";
import UserAvatar from "../../components/userInformation/UserAvatar";
import UserInfomation from "../../components/userInformation/UserInfomation";
import CommentItem from "../../features/commentQuestion/CommentItem";
import { formatTimeCreate } from "../../helps/dateformat";
import TableOfContents from "../../components/tableOfContent/TableOfContent";
import CommentInput from "../../features/commentQuestion/CommentInput";
import ViewAndComment from "../../features/favorite/ViewAndComment";
import LikeDislikeFavorite from "../../features/favorite/LikeAndDislike";
import { AuthContext } from "../../context/AuthContext";
import LikeDislikeQuestion from "../../features/favorite/LikeAndDislikeQuestion";

const QuestionDetail = () => {
  const { id } = useParams();
  const [question, setQuestion] = useState(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);
  const [user, setUser] = useState(null); // Changed from [] to null
  const [comments, setComments] = useState([]);
  const { accessToken } = useContext(AuthContext);

  useEffect(() => {
    const fetchQuestion = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/v1/question/${id}`
        );
        console.log("Fetched post response:", response);
        setQuestion(response.data.rs);
        setComments(
          Array.isArray(response.data.rs.comments)
            ? response.data.rs.comments
            : []
        );
        setIsLoading(false);
      } catch (error) {
        setError(error.response ? error.response.data.message : error.message);
        setIsLoading(false);
      }
    };

    fetchQuestion();
  }, [id]);

  const addComment = (newComments) => {
    setComments(newComments);
  };

  // Fetch user data based on post data
  useEffect(() => {
    const fetchUser = async () => {
      if (question?.idAuthor) {
        console.log("Fetching user data for idAuthor:", question.idAuthor);
        try {
          const response = await axios.get(
            `http://localhost:5000/api/v1/user/${question.idAuthor}`
          );
          console.log("User fetch response:", response);
          if (response.data.success) {
            setUser(response.data.rs);
          } else {
            console.error("Failed to fetch user:", response.data);
          }
        } catch (error) {
          console.error("Error fetching user:", error);
        }
      }
    };

    if (question) {
      fetchUser();
    }
  }, [question]);

  if (isLoading) return <div>Loading...</div>;
  if (error) return <div>Error: {error}</div>;

  return (
    <div className="max-w-[120rem] mx-auto p-4 pt-24">
      {question && (
        <div className="flex">
          {/* Table of Contents on the left */}
          <div className="w-1/6 pt-25 sticky space-y-8 pl-10">
            <LikeDislikeQuestion
              likes={question.likes || 0} // Assuming question.likes exists or defaults to 0
              dislikes={question.dislikes || 0} // Assuming question.dislikes exists or defaults to 0
              qid={id}
            />
          </div>
          {/* Main content */}
          <div cslassName="w-3/4 pr-4">
            <div className="flex items-center mb-4">
              <div className="flex items-center space-x-4">
                <UserAvatar src={user?.avatar} /> {/* Handling potential null user */}
                <UserInfomation
                  name={user?.firstname || "Unknown Author"} // Handle missing user data
                  timecreate={formatTimeCreate(question.createdAt)}
                />
                <ViewAndComment
                  views={question.views || 0}
                  comments={question.comments || 0}
                  addComment={addComment}
                />
              </div>
            </div>
            <h3 className="text-lg border p-4 font-semibold mb-1">
              {question.title}
            </h3>
            <div className="border border-gray-300 p-4 mb-4 rounded">
              <div dangerouslySetInnerHTML={{ __html: question.content }} />
            </div>
            {/* Handling tags array */}
            <div className="flex space-x-2 my-2">
              {question.tags.length > 0 ? (
                question.tags.map((tag, index) => (
                  <span
                    key={index}
                    className="bg-gray-200 text-gray-700 px-2 py-1 text-sm rounded"
                  >
                    {tag}
                  </span>
                ))
              ) : (
                <span className="text-sm text-gray-500">No tags available</span>
              )}
            </div>

            <div className="pb-5">
              <h1 className="py-2 font-bold text-lg">Comment</h1>
              <CommentInput
                id={id}
                accessToken={accessToken}
                addComment={addComment}
              />
            </div>

            <div className="space-y-4">
              {Array.isArray(comments) && comments.length > 0 ? (
                comments.map((comment, index) => (
                  <CommentItem
                    key={index}
                    user={comment.postedBy} // Use actual comment poster's info
                    _id={comment._id}
                    qid={id}
                    postedBy={comment.postedBy}
                    comment={comment.comment}
                    replies={comment.replies}
                    dateCreate={formatTimeCreate(comment.dateCreate)}
                  />
                ))
              ) : (
                <div>No comments yet.</div>
              )}
            </div>
          </div>
          {/* Table of Contents on the right */}
          <div className="w-1/4">
            <TableOfContents htmlContent={question.content} />
          </div>
        </div>
      )}
    </div>
  );
};

export default QuestionDetail;
